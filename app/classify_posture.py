...
LABELS_POS = open(drive_path+'darknet/posture_labels.names').read().strip().split('\n')
...
class PostureClassifier:
    ...
    def ForwardPassOutput(self, frame):
        ...
        return [LABELS_POS[class_lst[i]] for i in idx], [boxes[i] for i in idx], [confidences[i] for i in idx]
        
        
def drawBoxes(frame, labels, boxes, pig_ids):
    boxColor = (128, 255, 0) # very light green
    TextColor = (255, 255, 255) # white
    boxThickness = 3 
    textThickness = 2

    for lbl, box, id in zip(labels, boxes, pig_ids):
        start_coord = tuple(box[:2])
        w, h = box[2:]
        end_coord = start_coord[0] + w, start_coord[1] + h

    # text to be included to the output image
        txt = '{}  , ID: {} '.format(lbl, id)
...
def IsPigOnEdge(box, frame_shape = None, percent_win_edge = 10):
    # consider vertical dimension for now
    centers =  DetermineBoxCenter(box)
    PigOnEdge = centers[1] >= (frame_shape[0] * (1 - percent_win_edge/100)) 
...
class PigsInFrameTracker:            
    ...
    def TrackPigs(self, current_boxes): 
        ...
        if unmatched_idx.shape[0] > 0:  
            # check if these centers are real pigs, by checking if the distance is greater than 
            # the maximum dimension
            for i in unmatched_idx:
                if not IsPigOnEdge(current_boxes[i], self.frame_shape, percent_win_edge = 20):
            ...
            if len(new_centers) > 0:
                for i, (idx, nc) in enumerate(new_centers):
                    curr_boxes_dict[self.num_tracked_pigs + i] = {'box': current_boxes[idx], 
                    'center': list(nc.flatten())}

 
        PigsOnEdge = []     
        for k, v in curr_boxes_dict.items():
            if IsPigOnEdge(v['box'], self.frame_shape, percent_win_edge = 20): 
                PigsOnEdge.append(k) 
            
        for key in PigsOnEdge:
            del curr_boxes_dict[key]
        
        
        num_new_pigs = len(new_centers)
        self.num_tracked_pigs += num_new_pigs
...
if __name__ == '__main__':
   ...
   # set parameters 
   config_classification=  'darknet/cfg/custom-yolov4-posture_classify.cfg'
   wt_file_classificaton = 'backup/custom-yolov4-classification_best.weights'
   video_file ='data/colour_2_1_20160508T113001.108.mp4'  
   ...
   # Initialize objects
   tracker = PigsInFrameTracker(num_previous_frames = 10, frame_shape = (720, 1080))
   posture_classifier = PostureClassifier(wt_file_classificaton, config_classification, confidence = 0.3, nms_threshold=0.3)        
   ...
   while grab & (counter < num_frames_processed):
       ...
       'number of pigs': tracker.num_tracked_pigs})
       H, W = frame.shape[:2]
       if writer is None:
       ...
       if (((counter + 1) % int(fps // FRAME_RATE)) == 0) or (counter == 0):  
           labels, current_boxes, confidences, idx = posture_classifier.ForwardPassOutput(frame)
           frame = drawBoxes(frame, labels, current_boxes, confidences) 
           new_pig_count = tracker.TrackPigs(current_boxes)

           #print('new pig count = ', new_pig_count)
           frame = cv2.putText(frame, '{} {}'.format('total pig count:', str(new_pig_count)), 
