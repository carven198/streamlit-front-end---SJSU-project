...
LABELS = open(drive_path+'darknet/coco.names').read().strip().split('\n')
...
def drawBoxes(frame, labels, boxes, confidences):
    boxColor = (139, 0, 0) # dark red
    TextColor = (255, 255, 255) # white
    boxThickness = 2 
    textThickness = 2
    pig_ids = [i+1 for i in range(len(labels))]

    for lbl, box, conf, pig_id in zip(labels, boxes, confidences, pig_ids):
        start_coord = tuple(box[:2])
        w, h = box[2:]
        end_coord = start_coord[0] + w, start_coord[1] + h

    # text to be included to the output image
        txt = 'ID: {} '.format(pig_id)
...
class PigsInFrameTracker:            
    ...
    def TrackPigs(self, current_boxes): 
        if len(self.frames) == 0:
            ...
            return self.num_tracked_pigs, ids, current_boxes
...
if __name__ == '__main__':
   ...
   # set parameters 
   config =  'darknet/cfg/custom-yolov4-detector.cfg'
   wt_file = 'backup/custom-yolov4-detector_best.weights'
   ...
   while grab & (counter < num_frames_processed):
       ...
       if (((counter + 1) % int(fps // FRAME_RATE)) == 0) or (counter == 0):  
           labels, current_boxes, confidences, idx = obj_detector.ForwardPassOutput(frame)
